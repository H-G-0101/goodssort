 ...
 */
(function () {
  // valores das 4 opcoes e QUANTOS ads cada uma exige (estilo Bus Jam: n/N).
  var OPTIONS = [100, 250, 500, 1000];
  var ADS_REQ = [1, 2, 5, 9];          // +100=1 ad, +250=2, +500=5, +1000=9

  var ui = null, shown = false, hooked = false, adBusy = false;

  function g() { return window.__game; }
  function stats() { try { return g().data.stats; } catch (e) { return null; } }
  function prog() {  // progresso persistido no SAVE do jogo (sobrevive refresh)
    var st = stats(); if (!st) return [0, 0, 0, 0];
    if (!st.pcAdProg || !Array.isArray(st.pcAdProg) || st.pcAdProg.length !== 4) st.pcAdProg = [0, 0, 0, 0];
    return st.pcAdProg;
  }
  function save() {
  ...
  // ad REAL via CiDi (mesmo funil dos botoes nativos; overlay traduzido incluso)
  function playAd(cb) {
  ...
  function adBtn(i) {
    return '<button data-i="' + i + '" class="pcb-ad" style="display:flex;align-items:center;justify-content:center;gap:8px;min-width:104px;height:48px;padding:0 16px;border-radius:24px;cursor:pointer;font-family:inherit;border:2px solid #2e8f6b;background:linear-gradient(#5fd6a6,#2bb583 55%,#1f9f72);box-shadow:0 4px 10px rgba(30,140,98,0.35);">' +
      '<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><rect x="2.5" y="5" width="14" height="14" rx="3" fill="#fff"></rect><polygon points="7.5 9 7.5 15 12 12" fill="#2bb583"></polygon><path d="M16.5 9.5 21 7v10l-4.5-2.5z" fill="#fff"></path></svg>' +
      '<span class="pcb-cnt" style="font-weight:800;font-size:18px;color:#fff;text-shadow:0 1px 2px rgba(0,0,0,0.25);letter-spacing:.5px;">0/' + ADS_REQ[i] + '</span>' +
      '</button>';
  }
  function row(i) {
  ...
  function build() {
    ...
    btns.forEach(function (b) {
      b.onclick = function () {
        if (adBusy) return;                       // anti double-tap (licao do 2248)
        adBusy = true;
        var i = parseInt(b.getAttribute('data-i'), 10);
        b.disabled = true; b.style.filter = 'grayscale(0.4)';
        playAd(function (ok) {
          adBusy = false; b.disabled = false; b.style.filter = '';
          if (!ok) return;                        // ad falhou/fechado -> nao conta
          var st = stats(); if (!st) return;
          var p = prog();
          p[i] = (p[i] || 0) + 1;
          if (p[i] >= ADS_REQ[i]) {               // completou o pacote -> concede
            p[i] = 0;
            st.coins = (st.coins || 0) + OPTIONS[i];
            try { if (st.audio && g().sounds && g().sounds.collect) g().sounds.collect.play(); } catch (e) {}
            // pulso no contador ao conceder
            try { b.querySelector('.pcb-cnt').animate([{transform:'scale(1)'},{transform:'scale(1.4)'},{transform:'scale(1)'}],{duration:280}); } catch (e) {}
          }
          save();
          renderBalance(); renderProgress();
        });
      };
    });
  }
  ...
  function renderProgress() {
    if (!ui) return;
    var p = prog();
    ui.querySelectorAll('.pcb-ad').forEach(function (b) {
      var i = parseInt(b.getAttribute('data-i'), 10);
      var el = b.querySelector('.pcb-cnt');
      if (el) el.textContent = (p[i] || 0) + '/' + ADS_REQ[i];
    });
  }
  ...
  function show() { build(); renderBalance(); renderProgress(); ui.style.display = 'flex'; shown = true; hidePhaser(true); }
